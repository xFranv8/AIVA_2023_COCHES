...
class TestImagePatcher(unittest.TestCase):
    def setUp(self):
        self.car_detector = ImagePatcher((1024, 1024, 3), (512, 512, 3))

    def test_patcher(self):
        image: np.array = np.array(cv2.imread("test.png"))

        # Prueba: Número de patches correcto
        self.assertEqual(image.shape[0]/512, len(self.car_detector.patch_image(image)))

        # Prueba: Dimensión de la imagen reconstruida = Dimensión imagen original
        reconstructered_images = self.car_detector.unpatch_images(self.car_detector.patch_image(image))
